    ...
    public static class Utility
    {
        ...
        public static void WriteToTextFile(string path, string data, bool append = true, int count = 1)
        {
            ...
            using (StreamWriter writer = new StreamWriter(path, append: append))
            {
                if (!append)
                {
                    data = data.Trim().Substring(1, data.Trim().Length - 1); //removing the opening barcket '[' from data passed
                    data = data.Trim().Substring(0, data.Trim().Length - 1); //removing last bracket ']' from data passed
                }
                if (count != 0)
                {
                    data = data + ",";
        ...
        {
            PropertyDescriptorCollection property =
